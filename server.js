...
app.get("/call-history", (req, res) => res.json(callHistory.slice(-50).reverse()));

function history(type, fromName, toName, mode) {
  callHistory.push({ type, mode: mode || "voice", fromName: fromName || "Unknown", toName: toName || "Unknown", time: new Date().toLocaleString() });
}

function listUsers() {
  return Array.from(users.entries()).map(([socketId, u]) => ({ socketId, name: u.name, photo: u.photo || "", busy: u.busy }));
}

function sendUsers() { io.emit("users:update", listUsers()); }
...
io.on("connection", socket => {
  ...
  socket.on("call:accept", ({ to, answer, mode }) => {
    ...
    socket.to(to).emit("call:accepted", { from: socket.id, answer, mode });
  });
  ...
  socket.on("call:end", ({ to, mode }) => {
    ...
    sendUsers();
    if (to) socket.to(to).emit("call:ended", { from: socket.id });
  });
